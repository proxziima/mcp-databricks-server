...
def format_query_results(result: Dict[str, Any]) -> str:
    ...
    if result.get("status") == "success" and "data" in result:
        print(
            "Formatting results from SDK-based execute_databricks_sql output.",
            file=sys.stderr,
        )
    ...
    # Try to parse as old direct API style output (from dbapi.execute_statement)
        ...
        print(
            "Formatting results from original dbapi.execute_statement output.",
            file=sys.stderr,
        )
    ...
    output_lines.append(
        "-" * (sum(len(name) + 3 for name in column_names) - 1 if column_names else 0)
    )
