...
def _get_notebook_id_cached(notebook_path: str) -> str:
    ...
    if notebook_path not in _notebook_cache:
        try:
            ...
            print(f"Error fetching notebook {notebook_path}: {e}", file=sys.stderr)
...
def _process_lineage_results(
    ...
    """
    print("Processing lineage results with optimization...", file=sys.stderr)
    ...
    print(f"Pre-loading {len(unique_job_ids)} unique jobs...", file=sys.stderr)
    ...
    print(f"Job batch loading took {batch_time:.2f} seconds", file=sys.stderr)

    # Now process all notebook-job pairs using cached data
    print(f"Processing {len(notebook_job_pairs)} notebook entries...", file=sys.stderr)
    ...
    print(f"Total lineage processing took {total_time:.2f} seconds", file=sys.stderr)
...
def get_uc_schema_details(
    ...
    try:
        ...
        tables_iterable = sdk_client.tables.list(
